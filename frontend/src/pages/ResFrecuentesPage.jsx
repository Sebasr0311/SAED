...
export default function ResFrecuentesPage() {
  ...
  function validateQr() {
    ...
    if (qrForm.medioTransporte === 'BICICLETA' || qrForm.medioTransporte === 'OTRO') {
      if (!qrForm.descripcion.trim()) {
        e.descripcion = 'La descripci�n es requerida para ' + (qrForm.medioTransporte === 'BICICLETA' ? 'bicicleta' : 'otro medio');
      }
    }
    setQrErrors(e);
  ...
  async function save() {
    ...
    try {
      ...
      toast.success('Visitante creado. Para marcarlo como frecuente, genere un QR de "Visita R�pida" en /res-visita');
      setForm({
        idTipoDoc: 1,
        numeroDocumento: '',
        nombres: '',
  ...
  const columns = [
    ...
    { key: 'ultimaVisita', label: '�ltimo Ingreso', render: (r) => formatDate(r.ultimaVisita) },
  ];
  ...
  return (
    <div>
        ...
        {filtrados.map((f) => (
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {f.ultimaVisita && <div className="meta">�ltima visita: {formatDate(f.ultimaVisita)}</div>}
            </div>
        ...
        {qrGenerado ? (
          <div style={{ textAlign: 'center', padding: '8px 0' }}>
            ...
            <div style={{ fontSize: '12px', opacity: 0.7 }}>
              Compart� este c�digo con {qrModal?.nombreVisitante} para que el portero lo escanee.
          ...
          <>
            ...
            <div className="form-row">
              ...
              {(qrForm.medioTransporte === 'BICICLETA' || qrForm.medioTransporte === 'OTRO') && (
                <Input
                  id="qr-descripcion"
                  label={qrForm.medioTransporte === 'BICICLETA' ? 'Descripci�n de la bicicleta' : 'Descripci�n del medio'}
            ...
            <div className="form-group">
              <Input
                id="qr-notas"
                label="Motivo / descripci�n (opcional)"
                value={qrForm.notas}
      ...
      <ConfirmDialog
        ...
        message={`�Quitar a ${confirmQuitar?.nombreVisitante || ''} de tus visitantes frecuentes? Podr�s volver a generarle un QR m�s adelante.`}
        confirmLabel="Quitar"
        danger
      />
