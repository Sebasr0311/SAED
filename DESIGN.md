...
| `surface-selected` | `#DCE8FB` / dark `#16233C` | Selected row / item background |
| `preview-bg` | `#0B1220` | Photo / QR preview background |
| `chart-1..5` | navy & blue scale (light/dark) | Chart / donut palette from tokens |
| `on-surface` / `--on-surface` | `#0F172A` | Primary text on surface |
| `on-surface-variant` / `--on-surface-variant` | `#475569` | Secondary text |
| `text-muted` / `--text-muted` | `#64748B` | Hints, placeholders, empty states (AA 4.75:1) |
