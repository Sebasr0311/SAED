...
public class ProductionSchemaInitializer implements ApplicationRunner {
    ...
    private void initIncidentesPipeline() {
        try {
            ...
            // 3. Compilar PKG_SAED_SECURITY_RLS con soporte RLS para INCIDENTE_INVOLUCRADOS
            String rlsPlsql = """
                CREATE OR REPLACE PACKAGE BODY PKG_SAED_SECURITY_RLS AS
                ...
                """;
            try {
                jdbcTemplate.execute(rlsPlsql);
                log.info("[SchemaInit] PKG_SAED_SECURITY_RLS actualizado con soporte RLS para INCIDENTE_INVOLUCRADOS.");
            } catch (Exception ex) {
                log.warn("[SchemaInit] Aviso al compilar PKG_SAED_SECURITY_RLS: {}", ex.getMessage());
            }
