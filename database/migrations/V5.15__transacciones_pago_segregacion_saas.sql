-- ============================================================================
-- SAED 2.0 - V5.15 Segregación de Transacciones Pago Wompi SaaS vs Propiedad
-- ============================================================================
-- GAP-F6-01 / GAP-F6-03: Segregación Financiera Plataforma SaaS vs Copropiedad
-- Permite que ID_UNIDAD sea NULL para cobros de plataforma SaaS (Membresías, Renovaciones, Upgrades, Onboarding)
-- Agrega columna ID_ORGANIZACION para trazabilidad multi-tenant de transacciones SaaS
-- ============================================================================
...
DECLARE
    ...
    v_has_org NUMBER;
BEGIN
    -- 1. Modificar ID_UNIDAD en TRANSACCIONES_PAGO para permitir NULL
    SELECT NULLABLE INTO v_nullable
    FROM USER_TAB_COLS
    WHERE TABLE_NAME = 'TRANSACCIONES_PAGO' AND COLUMN_NAME = 'ID_UNIDAD';

    IF v_nullable = 'N' THEN
        EXECUTE IMMEDIATE 'ALTER TABLE TRANSACCIONES_PAGO MODIFY (ID_UNIDAD NULL)';
    END IF;

    -- 2. Agregar ID_ORGANIZACION si no existe
    SELECT COUNT(1) INTO v_has_org
    FROM USER_TAB_COLS
    WHERE TABLE_NAME = 'TRANSACCIONES_PAGO' AND COLUMN_NAME = 'ID_ORGANIZACION';

    IF v_has_org = 0 THEN
        EXECUTE IMMEDIATE 'ALTER TABLE TRANSACCIONES_PAGO ADD (ID_ORGANIZACION NUMBER)';
        BEGIN
            EXECUTE IMMEDIATE 'ALTER TABLE TRANSACCIONES_PAGO ADD CONSTRAINT FK_TRANSPAGO_ORG FOREIGN KEY (ID_ORGANIZACION) REFERENCES ORGANIZACIONES(ID_ORGANIZACION) ON DELETE CASCADE';
        EXCEPTION
            WHEN OTHERS THEN NULL;
        END;
        BEGIN
            EXECUTE IMMEDIATE 'CREATE INDEX IX_TRANSPAGO_ORG ON TRANSACCIONES_PAGO (ID_ORGANIZACION)';
        EXCEPTION
            WHEN OTHERS THEN NULL;
        END;
    END IF;
...
-- 3. Actualizar PKG_SAED_SECURITY_RLS con soporte RLS para TRANSACCIONES_PAGO SaaS (ID_UNIDAD NULL)
