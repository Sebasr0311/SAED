-- =============================================================================
-- SAED 2.0 - V5.20: TRABAJADORES, VIGENCIA ARL Y AUTORIZACIÓN DE OBRAS (GAP-F9-05)
-- =============================================================================
...
-- 1. Agregar columna ARL_FECHA_AFILIACION a TRABAJADORES si no existe
DECLARE
    v_count NUMBER;
BEGIN
    SELECT COUNT(1) INTO v_count
    FROM USER_TAB_COLS
    WHERE TABLE_NAME = 'TRABAJADORES' AND COLUMN_NAME = 'ARL_FECHA_AFILIACION';
    
    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'ALTER TABLE TRABAJADORES ADD (ARL_FECHA_AFILIACION DATE)';
    END IF;
END;
/

-- 2. Asegurar restricción única en (ID_PROVEEDOR, ID_PERSONA)
DECLARE
    v_count NUMBER;
BEGIN
    SELECT COUNT(1) INTO v_count
    FROM USER_CONSTRAINTS
    WHERE CONSTRAINT_NAME = 'UQ_TRABAJADOR_PROV_PERSONA';
    
    IF v_count = 0 THEN
        BEGIN
            EXECUTE IMMEDIATE 'ALTER TABLE TRABAJADORES ADD CONSTRAINT UQ_TRABAJADOR_PROV_PERSONA UNIQUE (ID_PROVEEDOR, ID_PERSONA)';
        EXCEPTION
            WHEN OTHERS THEN
                NULL;
        END;
...
BEGIN
    SELECT COUNT(1) INTO v_count
    FROM USER_TAB_COLS
    WHERE TABLE_NAME = 'OBRA_TRABAJADORES' AND COLUMN_NAME = 'FECHA_AUTORIZACION';
    
    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'ALTER TABLE OBRA_TRABAJADORES ADD (FECHA_AUTORIZACION TIMESTAMP(6) WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP)';
    END IF;

    SELECT COUNT(1) INTO v_count
    FROM USER_TAB_COLS
    WHERE TABLE_NAME = 'OBRA_TRABAJADORES' AND COLUMN_NAME = 'FECHA_REVOCACION';
    
    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'ALTER TABLE OBRA_TRABAJADORES ADD (FECHA_REVOCACION TIMESTAMP(6) WITH TIME ZONE)';
    END IF;

    SELECT COUNT(1) INTO v_count
    FROM USER_TAB_COLS
    WHERE TABLE_NAME = 'OBRA_TRABAJADORES' AND COLUMN_NAME = 'AUTORIZADO_POR';
    
    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'ALTER TABLE OBRA_TRABAJADORES ADD (AUTORIZADO_POR NUMBER)';
    END IF;
END;
/

-- 4. Actualizar PKG_SAED_SECURITY_RLS completo con filtro de TRABAJADORES
CREATE OR REPLACE PACKAGE BODY PKG_SAED_SECURITY_RLS AS
    ...
    BEGIN
        ...
        -- Regla especial: TRABAJADORES no tiene id_propiedad; se filtra por la organización de sus proveedores
